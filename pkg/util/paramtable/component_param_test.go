...
func TestProxyQueryViewStreamingConfig(t *testing.T) {
	...
	assert.False(t, config.EnablePlainSearchStreaming.GetAsBool())
	...
	assert.NoError(t, base.Save(config.EnablePlainSearchStreaming.Key, "true"))
	...
	assert.True(t, config.EnablePlainSearchStreaming.GetAsBool())
	...
	t.Setenv("PROXY_QUERYVIEW_ENABLEPLAINSEARCHSTREAMING", "true")
	...
	assert.True(t, envConfig.EnablePlainSearchStreaming.GetAsBool())
