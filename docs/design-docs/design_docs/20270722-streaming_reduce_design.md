...
| Concept | Definition |
| --- | --- |
| Unit | The smallest indivisible value processed by one reduction operation: one row for `OrderedReduce`, or one `<group_key, aggregate_value>` pair for `GroupReduce`. |
| Chunk | One Query/Search protobuf RPC message emitted after its reducible Unit payload reaches or crosses the configured `B`-byte threshold. A Unit is never split across Chunks. |

`Unit.ByteSize()` measures the reducible data carried by that Unit:

| Reduction input | `Unit.ByteSize()` includes |
| --- | --- |
...
Unit is 30 bytes, the complete Unit is appended and the emitted Chunk contains
110 reducible-payload bytes.
...
- `childBuffers[i]` is a concurrent-safe, implementation-dependent data structure that stores Units. Each `childBuffers[i]` has a target capacity of `B + y` reducible-payload bytes, where `y` is the byte threshold for retrieving the next Chunk before the Buffer becomes empty. Because an incoming Chunk retains the complete Unit that crosses `B`, a Buffer may exceed this target by that Chunk's final Unit. This allows us to employ a double-buffer approach to provide flexibility between search latency and memory.
...
Recv():
    ...
    while not IsChunkReady(outputBuffer):
        ...
        oneReduceResult = ProduceNextUnits(
            readyBuffers,
            outputBuffer.RemainingBytes()
        )

        if oneReduceResult.IsEmpty():
            break
...
`outputBuffer.RemainingBytes()` returns
...
| --- | --- |
| Chunk | One existing Query/Search protobuf message. A Chunk may contain fewer than `B` reducible-payload bytes when input is exhausted, or exceed `B` by the complete final Unit that crosses the threshold. |
...
AllocBuffer():
    ...
    outputBuffer.byteLimit = B
    return outputBuffer

IsChunkReady(outputBuffer):
    return outputBuffer.ByteSize() >= B
...
GetReadyBuffers(childStreams, childBuffers):
    ...
    for idx in childStreams.Size():
        if childBuffers[idx].ByteSize() <= y and childRecvTasks[idx].IsEmpty():
            childRecvTasks[idx] = one task to Recv() one Chunk into childBuffers[idx]

    return childBuffers

ProduceNextUnits(readyBuffers, remainingBytes):
    winner = 0
...
| --- | --- | --- | --- | --- |
| M1A | Iterator Streaming Foundation | Implement `ReduceStream` for iterator requests using Plain Query and Plain ANN Search semantics. Establish QN/SN-to-Proxy bidirectional streaming. | Iterator requests only. Non-iterator requests remain on the batch path. Emit each Chunk after its reducible payload reaches or crosses `B` bytes without splitting its final Unit. | End-to-end streaming execution succeeds. With deliberately large topK, peak retained memory approaches Chunk bytes times child count rather than complete-result bytes times child count. |
